...
public class CuriosCosmeticInventoryProvider implements InventoryProvider {
	...
	public boolean isAvailable(ServerPlayer target) {
		return CuriosAccess.inventory(target)
				.map(handler -> handler.getCurios().values().stream()
						.anyMatch(stackHandler -> stackHandler.getCosmeticStacks().getSlots() > 0))
				.orElse(false);
	}

	@Override
	public InventoryType getLockType() {
		return InventoryType.CURIOS_COSMETIC;
	}

	@Override
	public AbstractContainerMenu createMenu(int syncId, ServerPlayer viewer, ServerPlayer target) {
		return new PlayerCuriosInventoryScreenHandler(syncId, viewer, target, true, getInteractPermission());
	}
	...
	public String getInteractPermission() {
		return "inv_view.interact.curios_cosmetic";
	}
}
