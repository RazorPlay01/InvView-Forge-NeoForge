...
public abstract class AbstractInventoryScreenHandler extends AbstractContainerMenu implements ITargetPlayerContainer {
	...
	protected final InventoryType lockType;
	protected final ServerPlayer viewer;
	protected final int inventorySize;
	protected final String interactPermission;

	protected AbstractInventoryScreenHandler(MenuType<?> type, int syncId, ServerPlayer viewer, ServerPlayer target, InventoryType lockType, int inventorySize, String interactPermission) {
		super(type, syncId);
	...
	public void removed(@NotNull Player player) {
		...
		if (targetPlayer != null) {
			...
			if (lockType == InventoryType.ENDER_CHEST) {
				targetPlayer.getEnderChestInventory().setChanged();
			}
			// Guardar datos del jugador objetivo
			ModTemplate.savePlayerData(((ServerPlayerAccesor) viewer).server(), targetPlayer);
		}
